...
function updateView(visit) {
	if ( $("#hiddenLinks #" + visit.username).length > 0 ){
		$("#hiddenLinks #" + visit.username).html(visit.username + ": " + "<a href='" + visit.url + "' target='_blank'>" + visit.url + "</a>");
	} else {
		$('#hiddenLinks').append("<div style='width: 100%; color: rgba(0,0,0,0.8); padding: 10px 0px; border-bottom: 1px solid rgba(0,0,0,0.4);' id='" + visit.username + "'>" + visit.username + ": " + "<a href='" + visit.url + "' target='_blank'>" + visit.url + "</a></div>");
	}
}
